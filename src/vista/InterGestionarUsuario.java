...
public class InterGestionarUsuario extends javax.swing.JInternalFrame {
    ...
    private void EnviarDatosUsuarioSeleccionado(String idUsuario) {
        try {
            Connection con = (Connection) Conexion.conectar();
            PreparedStatement pst = con.prepareStatement("select usuario_id, usuario_nombre, usuario_apellido, usuario_usuario, usuario_password, usuario_telefono from usuario where usuario_id = '" + idUsuario + "'");
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                txt_Rut.setText(rs.getString("usuario_id"));
                txt_Nombre.setText(rs.getString("usuario_nombre"));
                txt_Apellido.setText(rs.getString("usuario_apellido"));
                txt_Usuario.setText(rs.getString("usuario_usuario"));
                txt_Contraseña.setText(rs.getString("usuario_password"));
                txt_Telefono.setText(rs.getString("usuario_telefono"));
            }
            con.close();
        } catch (SQLException e) {
